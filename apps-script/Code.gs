...
function normalizePayloadEntries(payload) {
  ...
  return [{ caption: payload.caption, media_type: payload.media_type, scheduled_at: payload.scheduled_at, publish_mode: payload.publish_mode, files: payload.files || (payload.file ? [payload.file] : []) }];
}
