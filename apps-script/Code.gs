...
function createRows(payload) {
  ...
  const pageNames = normalizePageNames(payload);
...
function normalizePayloadEntries(payload) {
  ...
  return [{
    ...
    files: payload.files || (payload.file ? [payload.file] : [])
  }];
}

function normalizePageNames(payload) {
...
function validateEntry(entry, pageNames) {
  ...
  if (!["NOW", "SCHEDULED"].includes(entry.publish_mode)) {
    throw new Error("publish_mode must be NOW or SCHEDULED.");
  }
  if (entry.publish_mode === "SCHEDULED" && !entry.scheduled_at) {
    throw new Error("scheduled_at is required for SCHEDULED mode.");
  }
  if (entry.media_type === "video" && Array.isArray(entry.files) && entry.files.length > 1) {
    throw new Error("Video accepts only one file.");
  }
}
...
function getNextIds(values, headers) {
  ...
  return {
    ...
    nextContentNumber: maxContentNumber + 1,
